...
if (turtle.xcor() >= TARGET_LEFT_X and
    ...
    # Подсказки по углу
    if y < TARGET_LEFT_Y:
        print('Снаряд не долетел по высоте, увеличьте угол!')
    elif y > (TARGET_LEFT_Y + TARGET_HEIGHT):
        print('Снаряд перелетел по высоте, уменьшите угол!')
    
    # Подсказки по силе
    if x < TARGET_LEFT_X:
        print('Снаряд не долетел по расстоянию, увеличьте силу!')
    elif x > (TARGET_LEFT_X + TARGET_WIDTH):
        print('Снаряд перелетел по расстоянию, уменьшите силу!')
